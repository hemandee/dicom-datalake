...
```bash
dicomParser$ sam local invoke HelloWorldFunction --event events/event.json
```

Don't forget to update the event.json detail with the fields you want to set from your schema.aws.s3.awsapicallviacloudtrail.AWSAPICallViaCloudTrail object
...
```bash
dicomParser$ sam logs -n HelloWorldFunction --stack-name dicomParser --tail
```
...
```bash
dicomParser$ pip install pytest pytest-mock --user
dicomParser$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name dicomParser
```
