...
def deleteSQSMsg(QUrl="default",RHandle="default",messageid="default"):
    sqs = boto3.client('sqs')
    try:
        sqs = boto3.client('sqs')
        response = sqs.delete_message(
            QueueUrl = QUrl,
            ReciptHandle=RHandle
        )
        logger.debug(f"Removed message id {messageid} from SQS {QUrl}")
    except Exception as e:
        logger.error("Unable to Delete SQS Message")
        logger.exception(e)
        raise
